...
namespace Sharp7
{
	...
	class MsgSocket
	{
		...
		{
			if (_tcpSocket == null) return S7Consts.errTCPSocketCreation;
		...
		public int Send(byte[] Buffer, int Size)
		{
			...
			try
			{
				int BytesSent = _tcpSocket.Send(Buffer, Size, SocketFlags.None);
			}
			catch
			{
				LastError = S7Consts.errTCPDataSend;
	...
	public class S7Client
	{
		...
		{           
			...
			try
			{
				_LastError = Socket?.Connect(IPAddress, _PLCPort) ?? S7Consts.errTCPConnectionFailed;
			}
			catch
			{
		...
		{
			if (Connected)
				_LastError = Socket?.Receive(Buffer, Start, Size) ?? S7Consts.errTCPNotConnected;
			else
		...
		{
			_LastError = Socket?.Send(Buffer, Len) ?? S7Consts.errTCPNotConnected;
		}
